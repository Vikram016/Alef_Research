        ...
        <AnimatePresence>
          {showNav && (
            <motion.div
              ...
              className="shrink-0 flex items-center justify-between px-6 sm:px-10 md:px-16 py-4 border-t border-alef-gold/30 bg-alef-modal"
            >
              <button
                onClick={() => hasPrev && onChangeIndex(activeIndex - 1)}
                disabled={!hasPrev}
                className="flex items-center gap-1 font-medium italic text-sm sm:text-base text-alef-ink disabled:opacity-30 disabled:cursor-not-allowed hover:text-alef-gold-dark transition-colors"
              >
                <ChevronLeft size={18} />
              ...
              <span className="font-medium text-xs sm:text-sm text-alef-gold-dark">
